  ...
  return (
    <header
      className={`fixed w-full top-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-white/80 dark:bg-[#0a0a0c]/80 backdrop-blur-lg border-b border-black/5 dark:border-white/5 py-3 shadow-sm'
          : 'bg-transparent py-5'
      }`}
    >
        ...
        <nav className="hidden md:flex items-center gap-8">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="text-sm font-medium text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-purple-400 transition-colors"
            >
              {link.label}
            </Link>
          ))}
      ...
      {isOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-white dark:bg-[#121215] border-t border-black/5 dark:border-white/5 py-4 px-4 flex flex-col gap-1 shadow-xl">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              onClick={() => setIsOpen(false)}
              className="text-base font-medium text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-purple-400 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
            >
              {link.label}
            </Link>
          ))}
